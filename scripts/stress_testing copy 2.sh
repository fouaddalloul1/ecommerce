...
# Extract k6 results
# ابحث عن هذه الدالة واستبدلها
extract_k6_results() {
    local json_file=$1
    local test_name=$2

    if [ ! -f "$json_file" ]; then
        echo -e "${RED}❌ ملف $json_file غير موجود${NC}"
        return 1
    fi

    # الطريقة الصحيحة لاستخراج الأرقام من k6 JSON
    ...
    # إذا ما لقى الأرقام، جرب طريقة ثانية
    if [ -z "$p95" ]; then
    ...
    echo ""
    echo -e "${BLUE}📈 نتائج الاختبار:${NC}"
    echo -e "   زمن الاستجابة p95: ${YELLOW}${p95}s${NC}"
    echo -e "   زمن الاستجابة avg: ${YELLOW}${avg}s${NC}"
    echo -e "   عدد الطلبات: ${YELLOW}${total_requests}${NC}"
}
...
# ============================================================
test_sync() {
    print_header "🧪 اختبار SYNC (بدون Queues - الوضع القديم)"

    echo -e "${YELLOW}⚠️ هذا الاختبار سيغير QUEUE_CONNECTION إلى sync${NC}"
    echo "يعني: الـ PDF والإيميل حيتشغلوا داخل الطلب نفسه (ما حيكون في Horizon)"

    # ✅ التصحيح: تأكد من أنها تصير sync مش redis
    sed -i 's/QUEUE_CONNECTION=redis/QUEUE_CONNECTION=sync/' .env
    # 🔴 إذا كانت sync أصلاً، نحتاج نغير العكس
    # استخدم هذا الأمر لضبطها قسراً:
    ...
    # ✅ العودة إلى redis بعد الاختبار
    sed -i 's/QUEUE_CONNECTION=sync/QUEUE_CONNECTION=redis/' .env
    php artisan config:clear

    echo ""
    echo -e "${GREEN}✅ انتهى اختبار SYNC${NC}"
}
test_scenario() {
